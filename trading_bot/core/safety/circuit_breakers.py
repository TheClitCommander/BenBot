...
class CircuitBreakerManager:
    ...
    def _reset_daily_counters(self) -> None:
        ...
        with self._lock:
            self._daily_loss = 0.0
            self._daily_trades = 0
            # Don't reset consecutive losses as those span across days
    ...
    def register_profit(self, amount: float, trade_id: str = None) -> None:
        ...
        with self._lock:
            # Profits don't count against daily loss, but do count as trades
            self._daily_trades += 1
            # Reset consecutive loss counter
            if self._consecutive_losses > 0:
                logger.info(f"Reset consecutive losses counter from {self._consecutive_losses} to 0 after profit")
            self._consecutive_losses = 0
            
            # Check max trades threshold
            if self._daily_trades >= self.config["maxTradesPerDay"]:
