...
class SafetyManager:
    ...
    def _load_events(self) -> None:
        ...
        try:
            if os.path.exists(self.safety_db_path):
                with open(self.safety_db_path, 'r') as f:
                    events_data = json.load(f)
                    
                # Create SafetyEvent objects from the loaded data
                for event_data in events_data:
                    ...
                    event.timestamp = datetime.datetime.fromisoformat(event_data["timestamp"])
    ...
    def add_event(self, event: SafetyEvent) -> None:
        ...
        # Keep the list at a reasonable size (last 1000 events)
    ...
    def get_safety_status(self) -> Dict[str, Any]:
        ...
        """
        trading_allowed, reason = self.check_trading_allowed()
